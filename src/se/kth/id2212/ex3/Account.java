...
@NamedQueries({
		@NamedQuery(name = "deleteAccountWithName", query = "DELETE FROM Account acct WHERE acct.owner.name LIKE :ownerName"),
		@NamedQuery(name = "findAccountWithName", query = "SELECT acct FROM Account acct WHERE acct.owner.name LIKE :ownerName", lockMode = LockModeType.OPTIMISTIC) })
@Entity(name = "Account")
public class Account implements Serializable {
	...
	public void deposit(float value) throws RejectedException {
		...
		balance += value;
		System.out.println("Transaction: Account " + owner.getName()
				+ ": deposit: $" + value + ", balance: $" + balance);
	}
